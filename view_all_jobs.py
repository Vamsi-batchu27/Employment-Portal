...
from view_applications import open_applications_window

def open_all_jobs_window(user):
    window = ctk.CTkToplevel()
    window.title("All Posted Jobs")
    ...
    ctk.CTkLabel(header_frame, text="📋 All Posted Jobs", font=("Arial", 22, "bold"),
    ...
    def load_all_jobs():
        ...
        if not jobs:
            ctk.CTkLabel(scroll_frame, text="No jobs found.",
        ...
        for job in jobs:
            ...
            ctk.CTkLabel(card, text=f"Posted By: {job.get('employer_name', 'N/A')}", font=("Arial", 12),
                         text_color="#777").pack(anchor="w", padx=10)
